/* rfid.c - Version Méthodique (Write avec vérification ACK) */
...
/* Vitesse 500kHz pour la sécurité du signal */
...
static int rc522_hw_init_once(void) {
    ...
    if (!device_is_ready(z_spi) || !device_is_ready(z_gpio)) return -ENODEV;
    
    gpio_pin_configure(z_gpio, CS_PIN, GPIO_OUTPUT_ACTIVE);
    cs_release(); 
    gpio_pin_configure(z_gpio, RST_PIN, GPIO_OUTPUT_ACTIVE);
    gpio_pin_set(z_gpio, RST_PIN, 1); 
    return 0;
}
...
void AntennaOn(void) {
    if (!(Read_MFRC522(TxControlReg) & 0x03)) SetBitMask(TxControlReg, 0x03);
}
...
uint8_t MFRC522_ToCard(uint8_t command, uint8_t *sendData, uint8_t sendLen, uint8_t *backData, unsigned short *backLen) {
    ...
    k_busy_wait(100); /* Delai de stabilisation */
...
uint8_t MFRC522_Anticoll(uint8_t *serNum) {
    ...
    for(int i=0; i<5; i++) serNum[i] = buffer[i];
    return MI_OK;
}
...
uint8_t MFRC522_SelectTag(uint8_t *serNum) {
    ...
    for (int i = 0; i < 5; i++) buf[2 + i] = serNum[i];
...
uint8_t MFRC522_Auth(uint8_t authMode, uint8_t BlockAddr, uint8_t *Sectorkey, uint8_t *serNum) {
    ...
    for (int i = 0; i < 6; i++) buf[2 + i] = Sectorkey[i];
    for (int i = 0; i < 4; i++) buf[8 + i] = serNum[i];
    unsigned short len = 0;
    
    uint8_t status = MFRC522_ToCard(PCD_AUTHENT, buf, 12, NULL, &len);
...
uint8_t MFRC522_Read(uint8_t blockAddr, uint8_t *recvData) {
    ...
    if (len >= 16) {
        for (int i = 0; i < 16; i++) recvData[i] = buffer[i];
        return MI_OK;
    }
    return MI_ERR;
}
...
/* * ÉCRITURE BLINDÉE
 * On vérifie que la carte répond ACK (4 bits) avant d'envoyer les données
 */
uint8_t MFRC522_Write(uint8_t blockAddr, uint8_t *writeData) {
    ...
    printk("RFID: Demande Write Bloc %d...\n", blockAddr);
    if (MFRC522_ToCard(PCD_TRANSCEIVE, buf, 4, ack_buf, &len) != MI_OK) {
        printk("RFID: Write CMD Fail (Carte refuse ou lecture seule)\n");
        return MI_ERR;
    }
    
    /* Vérif ACK (Mifare ACK est 4 bits, souvent 0x0A, mais le length doit être != 0) */
    if (len == 0 || (ack_buf[0] & 0x0A) != 0x0A) {
        /* Note: C'est permissif, certains clones répondent différemment, on continue si MI_OK */
        // printk("RFID: Warning ACK non standard: %02X\n", ack_buf[0]); 
    }

    /* 2. Envoi des 16 octets + CRC */
    uint8_t frame[18];
    for (int i = 0; i < 16; i++) frame[i] = writeData[i];
    CalulateCRC(frame, 16, &frame[16]);
    
    len = sizeof(ack_buf);
    printk("RFID: Envoi Data...\n");
    return MFRC522_ToCard(PCD_TRANSCEIVE, frame, 18, ack_buf, &len);
}
